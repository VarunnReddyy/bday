...
#         video_path = "memory lane1.mp4"  # Path to your local video file
...
#         video_splwishes_path="splwishes1.mp4"
...
if time_difference.total_seconds() > 0:
    st.title("Countdown to Birthday")
    countdown_text = f"Countdown to Birthday: {str(time_difference).split('.')[0]}"
    st.markdown(f'<div class="countdown">{countdown_text}</div>', unsafe_allow_html=True)
    st.write("Access will be granted once the countdown reaches zero.")
else:
    # Create a session state variable to track login status
    if 'authenticated' not in st.session_state:
        st.session_state.authenticated = False
    ...
    def login():
        ...
        # Ask a fun question as a text input instead of MCQ
        st.write("To enter, answer this question correctly:")
        question = st.text_input("Whom do I admire the most? (enter both first and last name with a space in between)")
